...
impl SstFileWriter {
    pub fn new(env_opt: &EnvOptions, opt: &Options) -> SstFileWriter {
        unsafe {
            SstFileWriter {
                inner: librocksdb_sys::rocksdb_sstfilewriter_create(env_opt.inner, opt.inner),
            }
        }
    }
    ...
    pub fn open(&mut self, name: &str) -> Result<(), String> {
        ...
        unsafe { Ok(ffi_try!(rocksdb_sstfilewriter_open(self.inner, path.as_ptr()))) }
    }
    ...
    pub fn add(&mut self, key: &[u8], val: &[u8]) -> Result<(), String> {
        unsafe {
            ffi_try!(rocksdb_sstfilewriter_add(self.inner,
                                               key.as_ptr(),
                                               key.len(),
                                               val.as_ptr(),
                                               val.len()));
    ...
    pub fn finish(&mut self) -> Result<(), String> {
        unsafe {
            ffi_try!(rocksdb_sstfilewriter_finish(self.inner));
            Ok(())
        }
    }
}
...
impl Drop for SstFileWriter {
    fn drop(&mut self) {
        unsafe { librocksdb_sys::rocksdb_sstfilewriter_destroy(self.inner) }
    }
}
