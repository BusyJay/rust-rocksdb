...
extern "C" {
    ...
    // EnvOptions
    pub fn rocksdb_envoptions_create() -> *mut EnvOptions;
    pub fn rocksdb_envoptions_destroy(opt: *mut EnvOptions);

    // IngestExternalFileOptions
    pub fn rocksdb_ingestexternalfileoptions_create() -> *mut IngestExternalFileOptions;
    pub fn rocksdb_ingestexternalfileoptions_set_move_files(opt: *mut IngestExternalFileOptions,
                                                            move_files: bool);
    ...
    pub fn rocksdb_ingestexternalfileoptions_destroy(opt: *mut IngestExternalFileOptions);

    // SstFileWriter
    pub fn rocksdb_sstfilewriter_create(env: *mut EnvOptions,
                                        io_options: *const DBOptions)
                                        -> *mut SstFileWriter;
    pub fn rocksdb_sstfilewriter_create_with_comparator(env: *mut EnvOptions,
                                                        io_options: *const DBOptions,
                                                        comparator: *const DBComparator)
                                                        -> *mut SstFileWriter;
    pub fn rocksdb_sstfilewriter_open(writer: *mut SstFileWriter,
                                      name: *const c_char,
                                      err: *mut *mut c_char);
    pub fn rocksdb_sstfilewriter_add(writer: *mut SstFileWriter,
                                     key: *const u8,
                                     key_len: size_t,
                                     val: *const u8,
                                     val_len: size_t,
                                     err: *mut *mut c_char);
    pub fn rocksdb_sstfilewriter_finish(writer: *mut SstFileWriter, err: *mut *mut c_char);
    pub fn rocksdb_sstfilewriter_destroy(writer: *mut SstFileWriter);

    pub fn rocksdb_ingest_external_file(db: *mut DBInstance,
                                        file_list: *const *const c_char,
                                        list_len: size_t,
                                        opt: *const IngestExternalFileOptions,
                                        err: *mut *mut c_char);
    pub fn rocksdb_ingest_external_file_cf(db: *mut DBInstance,
...
mod test {
    ...
    unsafe fn check_get(db: *mut DBInstance,
        ...
        let mut val_len = 0;
        let mut err = ptr::null_mut();
        let res_ptr = rocksdb_get(db, opt, key.as_ptr(), key.len(), &mut val_len, &mut err);
        assert!(err.is_null());
    ...
    fn test_ingest_external_file() {
        unsafe {
            ...
            let env_opt = rocksdb_envoptions_create();
            let io_options = rocksdb_options_create();
            let writer = rocksdb_sstfilewriter_create(env_opt, io_options);
            ...
            rocksdb_sstfilewriter_open(writer, c_sst_path_ptr, &mut err);
            assert!(err.is_null(), error_message(err));
            rocksdb_sstfilewriter_add(writer, b"sstk1".as_ptr(), 5, b"v1".as_ptr(), 2, &mut err);
            assert!(err.is_null(), error_message(err));
            rocksdb_sstfilewriter_add(writer, b"sstk2".as_ptr(), 5, b"v2".as_ptr(), 2, &mut err);
            assert!(err.is_null(), error_message(err));
            rocksdb_sstfilewriter_add(writer, b"sstk3".as_ptr(), 5, b"v3".as_ptr(), 2, &mut err);
            assert!(err.is_null(), error_message(err));
            rocksdb_sstfilewriter_finish(writer, &mut err);
            assert!(err.is_null(), error_message(err));

            let ing_opt = rocksdb_ingestexternalfileoptions_create();
            let file_list = &[c_sst_path_ptr];
            rocksdb_ingest_external_file(db, file_list.as_ptr(), 1, ing_opt, &mut err);
            assert!(err.is_null(), error_message(err));
            let roptions = rocksdb_readoptions_create();
            check_get(db, roptions, b"sstk1", Some(b"v1"));
            check_get(db, roptions, b"sstk2", Some(b"v2"));
            check_get(db, roptions, b"sstk3", Some(b"v3"));

            let snap = rocksdb_create_snapshot(db);

            fs::remove_file(sst_path).unwrap();
            rocksdb_sstfilewriter_open(writer, c_sst_path_ptr, &mut err);
            assert!(err.is_null(), error_message(err));
            rocksdb_sstfilewriter_add(writer, "sstk2".as_ptr(), 5, "v4".as_ptr(), 2, &mut err);
            assert!(err.is_null(), error_message(err));
            rocksdb_sstfilewriter_add(writer, "sstk22".as_ptr(), 6, "v5".as_ptr(), 2, &mut err);
            assert!(err.is_null(), error_message(err));
            rocksdb_sstfilewriter_add(writer, "sstk3".as_ptr(), 5, "v6".as_ptr(), 2, &mut err);
            assert!(err.is_null(), error_message(err));
            rocksdb_sstfilewriter_finish(writer, &mut err);
            assert!(err.is_null(), error_message(err));
            ...
            let roptions2 = rocksdb_readoptions_create();
            rocksdb_readoptions_set_snapshot(roptions2, snap);
            ...
            rocksdb_readoptions_destroy(roptions2);
